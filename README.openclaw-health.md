<p align="center">
  <img src="https://raw.githubusercontent.com/Sobranier/openclaw-doctor/main/assets/welcome.png" alt="OpenClaw health" width="400" />
</p>

<h1 align="center">OpenClaw health</h1>
...
```bash
npm install -g openclaw-health
openclaw-health watch -d
```
...
```bash
openclaw-health watch            # Start monitoring (foreground)
openclaw-health watch -d         # Start monitoring (background)
openclaw-health unwatch          # Stop monitoring

openclaw-health status           # Quick health check
```
...
```bash
openclaw-health gateway start    # Start the OpenClaw gateway
openclaw-health gateway stop     # Stop the gateway
openclaw-health gateway restart  # Restart the gateway
```

## Diagnostics & Logs

```bash
openclaw-health doctor           # Full diagnostics (binary, gateway, channels)
openclaw-health logs             # View gateway logs
openclaw-health logs --error     # View error logs only
openclaw-health logs --doctor    # View Doctor's own event logs
openclaw-health dashboard        # Web management UI (http://localhost:9090)
```
...
```bash
openclaw-health status --json    # Machine-readable output
openclaw-health watch -d         # Idempotent -- safe to call repeatedly
```
...
- **`openclaw-health`** — the main package (`package.json`)
- **`openclaw-health`** — alias package (`package.openclaw-health.json`)
...
2. Publishes `openclaw-health` with the default `package.json`
3. Temporarily swaps in `package.openclaw-health.json`, publishes `openclaw-health`, then restores

To update the `openclaw-health` package metadata (description, keywords, bin name, etc.), edit `package.openclaw-health.json`. Keep `version` in sync — it's automatically picked up from whichever `package.json` is active during publish.
