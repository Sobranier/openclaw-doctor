<p align="center">
  <img src="https://raw.githubusercontent.com/Sobranier/openclaw-doctor/main/assets/welcome.png" alt="OpenClaw run" width="400" />
</p>

<h1 align="center">OpenClaw run</h1>
...
```bash
npm install -g openclaw-run
openclaw-run watch -d
```
...
```bash
openclaw-run watch            # Start monitoring (foreground)
openclaw-run watch -d         # Start monitoring (background)
openclaw-run unwatch          # Stop monitoring

openclaw-run status           # Quick health check
```
...
```bash
openclaw-run gateway start    # Start the OpenClaw gateway
openclaw-run gateway stop     # Stop the gateway
openclaw-run gateway restart  # Restart the gateway
```

## Diagnostics & Logs

```bash
openclaw-run doctor           # Full diagnostics (binary, gateway, channels)
openclaw-run logs             # View gateway logs
openclaw-run logs --error     # View error logs only
openclaw-run logs --doctor    # View Doctor's own event logs
openclaw-run dashboard        # Web management UI (http://localhost:9090)
```
...
```bash
openclaw-run status --json    # Machine-readable output
openclaw-run watch -d         # Idempotent -- safe to call repeatedly
```
...
- **`openclaw-run`** — the main package (`package.json`)
- **`openclaw-run`** — alias package (`package.openclaw-run.json`)
...
2. Publishes `openclaw-run` with the default `package.json`
3. Temporarily swaps in `package.openclaw-run.json`, publishes `openclaw-run`, then restores

To update the `openclaw-run` package metadata (description, keywords, bin name, etc.), edit `package.openclaw-run.json`. Keep `version` in sync — it's automatically picked up from whichever `package.json` is active during publish.
