<p align="center">
  <img src="https://raw.githubusercontent.com/Sobranier/openclaw-doctor/main/assets/welcome.png" alt="OpenClaw install" width="400" />
</p>

<h1 align="center">OpenClaw install</h1>
...
openclaw-install status           # Quick health check
```
...
openclaw-install gateway restart  # Restart the gateway
```
...
openclaw-install dashboard        # Web management UI (http://localhost:9090)
```
...
openclaw-install watch -d         # Idempotent -- safe to call repeatedly
```
...
- **`openclaw-install`** — the main package (`package.json`)
- **`openclaw-install`** — alias package (`package.openclaw-install.json`)
...
2. Publishes `openclaw-install` with the default `package.json`
3. Temporarily swaps in `package.openclaw-install.json`, publishes `openclaw-install`, then restores
